use crate::{
    dealias, int_repr, to_zig_ident, wasm_type, Direction, InterfaceGenerator, RustFlagsRepr,
};
use heck::*;
...
impl<'a, 'b> FunctionBindgen<'a, 'b> {
    ...
    fn record_lower(
        ...
        for field in record.fields.iter() {
            let name = to_zig_ident(&field.name);
    ...
    fn record_lift(
        ...
        for (field, val) in ty.fields.iter().zip(operands) {
            result.push_str(&to_zig_ident(&field.name));
            result.push_str(": ");
            result.push_str(val);
            result.push_str(",\n");
        }
...
impl Bindgen for FunctionBindgen<'_, '_> {
    ...
    fn return_pointer(&mut self, size: usize, align: usize) -> String {
        ...
        // }
        uwriteln!(self.src, "const ptr{tmp} = alloc(8);");
        format!("ptr{}", tmp)
    }
    ...
    fn emit(
        ...
        match inst {
            ...
            Instruction::StringLower { realloc } => {
                ...
                let result_ptr = format!("result{}", tmp - 2);
                // if realloc.is_none() {
                //     self.push_str(&format!("let {} = {};\n", val, operands[0]));
                // } else {
                ...
                // }
                // self.push_str(&format!(
                //     "std.mem.writeIntLittle(u32, slice[0..4], @intCast(@intFromPtr({}.ptr)));\n",
                //     operands[0]
                // ));
                // self.push_str(&format!(
                //     "std.mem.writeIntLittle(u32, slice[4..8], @intCast({}.len));\n",
                //     operands[0]
                // ));
            ...
            Instruction::StringLift => {
                let tmp = self.tmp();
                let ptr_as_usize = format!("ptrAsUsize{}", tmp);
                let ptr = format!("ptr{}", tmp);
                let len = format!("len{}", tmp);
                uwrite!(
                    self.src,
                    "const {ptr_as_usize}: usize = @intCast({});
                  const {ptr}: [*]u8 = @ptrFromInt({ptr_as_usize});
                  const {len}: u32 = @intCast({});
                  ",
                    operands[0],
                    operands[1]
                );
                uwriteln!(self.src, "const bytes{tmp} = {ptr}[0..{len}];",);
            ...
            Instruction::CallInterface { func, .. } => {
                ...
                match &func.kind {
                    FunctionKind::Freestanding => {
                        self.push_str(&format!("Guest.{}", &to_zig_ident(&func.name)));
                    }
            ...
            Instruction::I32Store { offset } => {
                self.push_str(&format!(
                    "std.mem.writeIntLittle(u32, {}[{offset}..{}], @intCast({}));\n",
                    operands[1],
                    offset + 4,
                    operands[0]
                ));
