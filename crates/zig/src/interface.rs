...
use crate::{
    dealias, int_repr, to_upper_camel_case, to_zig_ident, wasm_type, Direction, ExportKey, FnSig,
    Identifier, InterfaceName, Ownership, RustFlagsRepr, TypeMode, ZigWasm,
};
use anyhow::Result;
use heck::*;
...
impl InterfaceGenerator<'_> {
    ...
    pub fn start_append_submodule(&mut self, name: &WorldKey) -> (String, Vec<String>) {
        let snake = match name {
            WorldKey::Name(name) => to_zig_ident(name),
            WorldKey::Interface(id) => {
                to_zig_ident(self.resolve.interfaces[*id].name.as_ref().unwrap())
            }
        };
    ...
    fn generate_guest_export(&mut self, func: &Function) {
        ...
        uwrite!(
            self.src,
            "
                export fn __export_{name_snake}(\
            ",
        );
        ...
        if abi::guest_export_needs_post_return(self.resolve, func) {
            ...
            uwrite!(
                self.src,
                "
                    export fn __post_return_{name_snake}(\
                "
            );
            let mut params = Vec::new();
    ...
    pub fn generate_stub(
        ...
        let path = if let Some((namespace, pkg_name)) = pkg {
            format!(
                "{}::{}::{}",
                to_zig_ident(&namespace),
                to_zig_ident(&pkg_name),
                to_zig_ident(name),
            )
        } else {
            to_zig_ident(name)
    ...
    fn rustdoc_params(&mut self, docs: &[(String, Type)], header: &str) {
        ...
        //             if i == 0 {
        //                 self.push_str("* `");
        //                 self.push_str(to_zig_ident(param.name.as_str()));
        //                 self.push_str("` - ");
        //             } else {
        //                 self.push_str("  ");
        //             }
    ...
    fn print_docs_and_params(
        ...
        self.push_str(&to_zig_ident(func_name));
    ...
    fn print_typedef_record(
        ...
        for (name, mode) in self.modes_of(id) {
            ...
            for field in record.fields.iter() {
                ...
                self.push_str("pub ");
                self.push_str(&to_zig_ident(&field.name));
                self.push_str(": ");
